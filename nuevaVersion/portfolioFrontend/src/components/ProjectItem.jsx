...
function ProjectItem({ id, name, image, description, tools = [] }) {
      ...
      <div className="flex flex-col sm:flex-row p-4 gap-4 flex-wrap">
        ...
        <div className="flex flex-col flex-1 min-w-0 break-words">
          ...
          <Link
            ...
            className="bg-black text-white text-xs px-2 py-1 rounded-full hover:bg-gray-800  transition-colors duration-300 hover:scale-105 transition-transform duration-300 inline-block self-start text-center"
          >
            Read More
          </Link>
        </div>
        ...
        <div className="flex-1 min-w-0 text-gray-600 break-words">
          ...
          {tools.length > 0 && (
            <div className="mt-4">
              <h2 className="font-semibold text-gray-700 mb-2 text-sm">Tools:</h2>
              <ul className="flex flex-wrap gap-1 max-w-full overflow-hidden">
                {tools.map((tool, index) => (
                  <li
                    key={index}
                    className="bg-gray-200 px-1 py-0.5 rounded text-xs text-gray-800 truncate max-w-[100px] text-center"
                  >
                    {tool}
                  </li>
                ))}
