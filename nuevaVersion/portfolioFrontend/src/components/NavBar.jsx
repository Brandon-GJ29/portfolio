...
const NavBar = ({ t, toggleLanguage, i18n}) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-30">
      ...
      {/* Contenedor de la hamburguesa + botón */}
      <div className="fixed top-3 right-6 z-50 flex items-center gap-3">
        {/* Botón a la izquierda de la hamburguesa */}
        <div onClick={toggleLanguage} className={`w-8 h-4 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 ${
          i18n.language === "es" ? "bg-gray-200" : "bg-gray-100"
        }`}>
          <img
            src={i18n.language === "en" ? assets.mundoUSA : assets.mundoMex}
            alt="toggle icon"
            className={`w-4 h-4 rounded-full shadow-md transform duration-300 object-cover
              ${i18n.language === "es" ? "translate-x-0" : "translate-x-4"}`}
          />
          </div>

        {/* Icono de hamburguesa */}
        <div className="flex flex-col justify-between w-6 h-6 cursor-pointer" onClick={() => setMenuOpen(!menuOpen)}>
